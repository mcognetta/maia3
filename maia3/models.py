...
class RelativeBias(nn.Module):
    def __init__(self, nheads, name=None):
        ...
        self.register_buffer(
            "rpe_factorizer", torch.tensor(rpe_map, dtype=torch.float32)
        )
        self.nheads = nheads
    ...
    def __init__(
        ...
        *,
        ...
        if use_gab:
            ...
            self.sm3 = nn.Linear(gab_intermediate_dim, nhead * gab_gen_size)
...
class MAIA3Model(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        assert (
            self.cfg.use_gab + self.cfg.use_relative_bias + self.cfg.use_absolute_pe
        ) >= 1, (
            "At least one of use_gab, use_relative_bias, use_absolute_pe must be True"
        )
        ...
        # TODO(marco): Add back in time information here
        self.token_projection = nn.Linear(
            12 * cfg.history + 2 * cfg.dim_emb, cfg.dim_vit
        )

        if cfg.use_gab:
            self.gab_shared_weight = nn.Parameter(
                torch.empty(64 * 64, cfg.gab_gen_size)
            )
        ...
        self.promo_bias_proj = nn.Linear(
            cfg.head_hid_dim, 4, bias=False
        )  # 4 promotion types: q, r, b, n

        self.rank7_indices = [
            chess.square(file, 6) for file in range(8)
        ]  # squares 48-55
        self.rank8_indices = [
            chess.square(file, 7) for file in range(8)
        ]  # squares 56-63
    ...
    def interpolate_elo(self, elos):
        ...
        elo_emb_low = self.elo_embedding_low(
            torch.zeros_like(elos, dtype=torch.long)
        )  # (B, dim_emb)
        elo_emb_high = self.elo_embedding_high(
            torch.zeros_like(elos, dtype=torch.long)
        )  # (B, dim_emb)

        return (
            weight_low.unsqueeze(1) * elo_emb_low
            + weight_high.unsqueeze(1) * elo_emb_high
        )
    ...
    def forward(self, tokens, self_elos, oppo_elos):
        ...
        self_elo_embs = self_elo_embs.unsqueeze(1).expand(
            -1, 64, -1
        )  # (B, 64, dim_emb)
        oppo_elo_embs = oppo_elo_embs.unsqueeze(1).expand(
            -1, 64, -1
        )  # (B, 64, dim_emb)

        embs = torch.cat(
            [tokens, self_elo_embs, oppo_elo_embs], dim=-1
        )  # (B, 64, tokens_dim + 2*dim_emb)
        x = self.token_projection(embs)  # (B, 64, dim_vit)

        if self.cfg.use_absolute_pe:
            x = self.abs_pe(x)
        ...
        rank8_features = sq_to[:, 56:65, :]  # (B, 8, head_hid_dim)
        promo_biases = self.promo_bias_proj(rank8_features) * math.sqrt(
            self.cfg.head_hid_dim
        )  # (B, 8, 4) for q,r,b,n

        base = scores_base[:, 48:56][:, :, 56:65]  # (B, 8, 8)
        promotion_logits = (base.unsqueeze(-1) + promo_biases.unsqueeze(1)).reshape(
            x.size(0), 256
        )
        ...
        return logits_move, logits_value  # (B, 4352), (B, 3)
