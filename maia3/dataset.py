...
def build_move_vocab() -> tuple[dict[str, int], int]:
    """The model's fixed move vocabulary as ``{uci: index}`` plus its size."""
    all_moves = get_all_possible_moves()
    return {m: i for i, m in enumerate(all_moves)}, len(all_moves)
...
class ParquetPositionDataset(Dataset):
    ...
    def __init__(
        ...
        self.all_moves_dict = (
            all_moves_dict if all_moves_dict is not None else build_move_vocab()[0]
        )
